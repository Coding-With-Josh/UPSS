    ...
    >
      ...
      <div className="grid gap-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="grid gap-2">
            <Label>Class</Label>
            <Select onValueChange={(value) => form.setValue('class', value as any)} defaultValue={form.getValues('class')}>
              <SelectTrigger className={cn(form.formState.errors.class && "border-red-500")}>
          ...
          <div className="grid gap-2">
            <Label>Arm</Label>
            <Select onValueChange={(value) => form.setValue('arm', value as any)} defaultValue={form.getValues('arm')}>
              <SelectTrigger className={cn(form.formState.errors.arm && "border-red-500")}>
                <SelectValue placeholder="Select arm" />
              </SelectTrigger>
              <SelectContent>
                {['Silver', 'Gold', 'Platinum', 'Copper', 'Mecury', 'Diamond', 'Titanium', 'Silicon'].map((arm) => (
        ...
        <div className="grid gap-4 md:grid-cols-2">
            ...
            <Select onValueChange={(value) => form.setValue('role', value as any)} defaultValue={form.getValues('role')}>
              ...
              <SelectContent>
                {['STUDENT', 'TEACHER', 'ADMIN'].map((role) => (
                  <SelectItem key={role} value={role}>{role.toLowerCase()}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="grid gap-2">
            ...
            <Select onValueChange={(value) => form.setValue('gender', value as any)} defaultValue={form.getValues('gender')}>
              <SelectTrigger className={cn(form.formState.errors.gender && "border-red-500")}>
                <SelectValue placeholder="Select gender" />
              </SelectTrigger>
              <SelectContent>
                {['MALE', 'FEMALE', 'OTHER'].map((gender) => (
                  <SelectItem key={gender} value={gender}>{gender.toLowerCase()}</SelectItem>
                ))}
              </SelectContent>
            </Select>
